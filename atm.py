...
class Bank:
    def __init__(self,root):
        ...
        self.login = False
        self.root = root
        self.header = Label(self.root,text="STUDENT'S BANK",bg="#50A8B0",fg="white",font=("arial",20,"bold"))
        ...
        self.q.place(x=380,y=360,width=120,height=20)
        self.frame.pack()
    ...
    def MainMenu(self):#Main App Appears after logined !
        self.frame = Frame(self.root,bg="#728B8E",width=700,height=400)
        root.geometry("700x400")
        self.passwd= Button(self.frame,text="Pin change",bg="#50A8B0",fg="white",font=ARIAL,command=self.pass_change)
        ...
        self.q = Button(self.frame, text="Quit", bg="#50A8B0", fg="white", font=ARIAL, command=self.root.destroy)
        self.passwd.place(x=0,y=100,width=200,height=50)
        self.enquiry.place(x=0, y=200, width=200, height=50)
        self.deposit.place(x=500, y=100, width=200, height=50)
        self.withdrawl.place(x=500, y=200, width=200, height=50)
        self.q.place(x=280, y=340, width=120, height=20)
        self.frame.pack()

    def pass_change(self):
        self.pin_box = Entry(self.frame,bg="honeydew",highlightcolor="#50A8B0",
           highlightthickness=2,
            highlightbackground="white")
        self.submitButton = Button(self.frame,text="Submit",bg="#50A8B0",fg="white",font=ARIAL)
        self.pin_box.place(x=220,y=100,width=200,height=20)
        self.submitButton.place(x=435,y=100,width=55,height=20)
        self.submitButton.bind("<Button-1>",self.pass_comp)
    def pass_comp(self,flag):
        self.label = Label(self.frame, text="Password Updated !", font=ARIAL)
